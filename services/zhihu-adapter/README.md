...
| `GET /zhihu/user` | OAuth | 获取当前授权用户信息 ✓ |
| `GET /zhihu/following-feed` | OAuth | 获取关注动态 ✓ (184条) |
| `GET /zhihu/user-followed` | OAuth | 获取关注列表 ✓ (10个关注) |
| `GET /zhihu/user-followers` | OAuth | 获取粉丝列表 ✓ (10个粉丝) |

### OAuth 流程验证（2026-05-13）

OAuth 全链路已打通：

1. ✓ `GET /zhihu/oauth/authorize` - 获取授权 URL
2. ✓ `GET /zhihu/oauth/callback` - 授权回调，换取 access_token
3. ✓ 使用 access_token 调用所有 OAuth 数据接口

**测试用户**: 愚者三尺 (UID: 749275245789069312)
...
**技术要点**:
- OAuth token 通过 Query 参数 `access_token` 传递
- OAuthClient 内部使用 `Authorization: Bearer {token}` header 调用知乎API
- 分页参数: `page` (从0开始), `per_page` (最大50)
- 响应格式支持 `{"data": [...]}` 和裸数组两种
