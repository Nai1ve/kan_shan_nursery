...
- 官网地址：<https://developer.zhihu.com/>
...
### 2.1 获取 Access Secret

请在知乎开放平台个人中心查看并获取 `Access Secret`。

调用方需要将 `Access Secret` 作为 Bearer Token 放入请求头中。
...
> 注意：服务端会校验 `Authorization` 与 `X-Request-Timestamp`。
...
| 接口         | Method | URL                             | 说明             |
| ------------ | -----: | ------------------------------- | ---------------- |
| 知乎搜索 API |    GET | `/api/v1/content/zhihu_search`  | 知乎站内内容搜索 |
| 全网搜索 API |    GET | `/api/v1/content/global_search` | 全网内容搜索     |
| 直答 API     |   POST | `/v1/chat/completions`          | 知乎直答模型调用 |
| 知乎热榜 API |    GET | `/api/v1/content/hot_list`      | 获取当前知乎热榜 |
...
| HTTP Method | `GET`                                                     |
| 鉴权方式    | `Authorization: Bearer <your_access_secret>`              |
...
| 参数    | 类型     | 必填 | 默认值 | 限制      | 说明       |
| ------- | -------- | ---: | -----: | --------- | ---------- |
| `Query` | `String` |   是 |      - | 不能为空  | 查询关键词 |
| `Count` | `Int32`  |   否 |   `10` | 最大 `10` | 请求数量   |
...
| `EditTime`        | `Int64`              |       是 | 最后编辑时间戳，例如 `1745486539`                          |
| `CommentInfoList` | `Array[CommentInfo]` |       否 | 精选评论                                                   |
    ...
    "Items": [
      {
        ...
        "ContentText": "首先要澄清一个常见误解：ChatGPT的免费版和付费版使用的是不同模型与功能配置，体验差距确实很大。",
...
| 响应类型    | `application/json`（`stream=false`）或 `text/event-stream`（`stream=true`） |
| 鉴权方式    | `Authorization: Bearer <your_access_secret>`                 |
...
流式响应已经发出 HTTP 200 后，如果中途发生错误，返回形式如下：
...
该接口用于获取当前知乎热榜内容，返回结构化的标题、链接、缩略图与摘要列表。
...
| ------: | -------- | ------------------------------------------ | ----------------------------------------- |
|     `0` | 成功     | 请求正常                                   | 正常解析响应数据                          |
| `10001` | 参数错误 | 参数缺失、参数格式错误、参数超出限制       | 检查 Query、Count、Limit 等字段           |
| `20001` | 鉴权失败 | Access Secret 错误、请求头缺失、时间戳异常 | 检查 Authorization 与 X-Request-Timestamp |
| `30001` | 频率限制 | 请求过于频繁                               | 降低请求频率或增加重试退避                |
| `90001` | 内部错误 | 服务端异常                                 | 稍后重试，必要时联系平台方                |
...
建议在业务服务中封装统一的知乎开放平台客户端，至少包含以下能力：
...
- 不要将 `Access Secret` 写死在前端代码中。
- 不要将 `Access Secret` 提交到 Git 仓库。
- 生产环境建议通过环境变量、密钥管理服务或配置中心注入。
- 日志中避免打印完整 `Authorization` 请求头。
...
| 已获取 Access Secret                     |        ☐ |
| 请求头包含 `Authorization`               |        ☐ |
| 请求头包含 `X-Request-Timestamp`         |        ☐ |
| 时间戳为秒级 Unix 时间戳                 |        ☐ |
| `Content-Type` 设置为 `application/json` |        ☐ |
| GET 参数已正确 URL encode                |        ☐ |
| 已处理 `Code != 0` 的错误响应            |        ☐ |
| 已处理接口频率限制                       |        ☐ |
| 已避免在日志中泄露 Access Secret         |        ☐ |
